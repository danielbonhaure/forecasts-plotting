...
PlotsHelper <- R6::R6Class(
  ...
  public = list(
    ...
    definir_estilos_css = function() {
      ...
      css_info_adicional <-
        "#info-adicional {font: small-caps 100%/200% serif; font-style: italic; font-weight: 1000; font-size: 90%; color: orangered;}"
      # Convert CSS to HTML
      html_fix <- htmltools::tags$style(
        type = "text/css", paste(css_fix_1, css_fix_2, css_fix_3, css_title, css_logos,
                                 ...
                                 css_hide_button, css_fa_eye, css_fa_eye_slash, css_info_adicional))
    },
    ...
    definir_titulo = function(data_type, base_file, lang, data_year = NULL) {
      ...
      if (data_type == "anom") {
        ...
        main_title <- glue::glue("{anomaly_desc} {tolower(variable_str)} ({variable_unit}). ",
                                 "\n{valid_for} {month_year}. {init_cond} {initial_month} {initial_year}. ",
                                 "\n{calibrated} {modelo}.")
        ...
        prob_fcst_desc <- switch(lang, "en" = "Probabilistic forecast for the most likely category of", 
      ...
      # Obtener información adicional del archivo de configuración
      info_config <- global_config$get_config(base_file$type)$additional_info

      # En caso de ser necesario, agregar info adicional al título generado
      if ( info_config$display ) {
        # Obtener información adicional
        additional_info <- switch(lang, "en" = info_config$message$en,
                                        "es" = info_config$message$es,
                                        "pt" = info_config$message$pt)
        # Agregar información adicional al título
        main_title <- glue::glue(
          "{main_title}\n<span id='info-adicional'>{additional_info}</span>")
      }
